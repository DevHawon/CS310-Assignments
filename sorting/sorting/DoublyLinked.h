...
class DoublyLinked
{
    ...
    public:
        ...
        vector<int> indexOf(T val);
	
		// above methods are from the previous assignment.
		// insertionSort method is for deliverable 2.
		// it returns nothing
		void insertionSort();
...
// displaying all the elements of the list
template<typename T>
void DoublyLinked<T>::displayNode()
{
	Node<T> *current = headPtr;
...
// deletes all the elements of the linked list
template<typename T>
void DoublyLinked<T>::deleteItem()
{
    Node<T> *tempPtr;
...
bool DoublyLinked<T>::reverse()
{
    ...
    if(count == 0 ||    headPtr == nullptr || headPtr == nullptr)
    {
        return false;
    }
...
// search value and return all indices containing the equal value
template<typename T>
vector<int> DoublyLinked<T>::indexOf(T val)
{
    vector<int> indices;
...
// do insertionSort in ascending order
template<typename T>
void DoublyLinked<T>::insertionSort()
{
	// when the list contains nothing or one node
    if(count < 2)
    {
	...
	Node<T> *currentNode = headPtr->next;
	Node<T> *tempNode;
	int key;
	...
	while(currentNode->next != nullptr)
	{
		...
		while(tempNode != nullptr && tempNode->data > key)
		{
			tempNode->next->data = tempNode->data;
			
			tempNode = tempNode->prev;
		}
		tempNode->next->data = key;
